...
parser.add_argument("--cell_line", default="LNCaP",help = "Cell line to be simulated.") #, choices=["22Rv1", "BHP1", "DU145", "PC3", "LNCaP", "VCaP", "AGS"], )
...
# Process arguments
####################################################################
#python3 physiboss_drugsim.py -p prostate --cell_line LNCaP -d Ipatasertib,Afatinib,Ulixertinib,Luminespib,Selumetinib,Pictilisib --mode both --drug-concs IC10 IC30 IC50 IC70 IC90
project=args.project
cell_line = args.cell_line
drug_concs = args.drug_concs
drugs=args.drugs

drug_list = str(drugs.replace (" ", "")).split(",")
drugslist_csv = pd.read_csv("./config/drugs.csv", index_col=0)
drugslist_csv = drugslist_csv.loc[drug_list]
print("Drugs to be administered: "+str(drug_list).replace("[", "").replace("]","").replace("'","")) #Estos replace estan puestos para hacer bonito.
# convert druglist into node-list
if (project == "prostate") :
    node_list = [drugslist_csv.at[drug,"node"] for drug in drug_list]
else :
    node_list = [drug_node_pairs_AGS.get(item,item)  for item in drug_list]

# specify boolean model path
bool_model_path_dir = "{}/{}".format("config", "boolean_network")
if (project == "prostate") :
...
node_list_1 = [x.split(", ") for x in node_list] #Lista de listas de un elemento cada una

# set drug sensitivity path
drug_csv_path =  "{}/{}_{}".format("config", project,"drug_sensitivity.csv")
...
def get_param_from_drug_df(drug_name, drug_df, param, drugslist):
    drug_id = drugslist.at[drug_name,"drug_id"]
...
def add_drug_to_xml(drug, drug_concs, rest, path_to_xml, xml_output_path, model_name, mode, output_path, druglist_csv):
    ...
    scal = get_param_from_drug_df(drug, drug_dataframe, "scal", druglist_csv)
    xmid = get_param_from_drug_df(drug, drug_dataframe, "xmid", druglist_csv)
    maxc = get_param_from_drug_df(drug, drug_dataframe, "maxc", druglist_csv) 
...
def setup_drug_simulations(druglist, nodelist, bool_model_name, bool_model, rest_list, drug_concs, mode, simulation_list, druglist_csv, cell_line=cell_line):
    ...
    #check if output base path exists, if not create it
    ...
    for drug in druglist:
        ...
        for rest in rest_list:
            ...
            for drug_conc in drug_concs:
                ...
                output_path = "{}/{}_{}_{}_{}".format(output_base_path, cell_line, filtered_drug_conc.replace(",", "_"), filtered_drugname.replace(",","_"), filtered_rest.replace(",","_"))
                ...
                if (type(drug) is tuple):
                    ...
                    add_drug_to_xml(drug[0], drug_conc[0], rest[0],  xml_path, new_xml_output_path, bool_model_filename, mode, output_path, druglist_csv)
                    add_drug_to_xml(drug[1], drug_conc[1], rest[1], new_xml_output_path, new_xml_output_path, bool_model_filename, mode, output_path, druglist_csv)
                else: 
                    add_drug_to_xml(drug, drug_conc, rest, xml_path, new_xml_output_path, bool_model_filename, mode, output_path, druglist_csv)
                xml_config_path = "{}/{}_{}_{}_{}.{}".format("config", cell_line, filtered_drug_conc.replace(",", "_"), filtered_drugname.replace(",","_"), filtered_rest.replace(",","_"), "xml")

####################################################################
# Set up config files for drug simulations
####################################################################

mode = args.mode
print(mode + " drug simulations will be set up.")
simulation_list = []
if (mode == "single" or mode == "double"):
    ...
    if os.path.exists("config/single"):
        shutil.rmtree("config/single")    
    os.makedirs("config/single")
    if os.path.exists("config/double"):
        shutil.rmtree("config/double")
    os.makedirs("config/double")
    setup_drug_simulations(drug_list, node_list, bool_model_filename, bool_model, drug_rest_name_list, drug_concs, "single", simulation_list, drugslist_csv)
    setup_drug_simulations(drug_list, node_list, bool_model_filename, bool_model, drug_rest_name_list, drug_concs, "double", simulation_list, drugslist_csv)